...
/**设置Unity接收返回数据的函数*/
-(void)SetUnityFuncName
{
    m_pstrFuncName = @"OnAvaterCallBack";
//    [m_pstrFuncName retain];
}
/**设置保存图片的名字*/
-(void)SetUnityFileName
{
  //  NSLog(@"m_pstrFileName 1:%@",pstr);
    m_pstrFileName = @"/image.png";
 //   m_pstrFileName = [NSString stringWithFormat:@"%@%@",@"/",m_pstrFileName];
  // NSLog(@"m_pstrFileName 2:%@",m_pstrFileName);
...
extern "C"{
#endif
    void SettingAvaterFormiOS(char* pstrObjectName,char* pstrFuncName,char*pstrFileName )
{
    ...
    [app SetUnityFuncName];
    [app SetUnityFileName];
    [app showActionSheet];
