...
public class ExcelExporterEditor : EditorWindow
{
    ...
    private const string ClassPath = @"Hotfix\Module\CSV";
    ...
    private void OnGUI()
    {
        try
        {
            ...
            if (GUILayout.Button("生成Csv配置类"))
            {
                ExportAllCsvClass(ClassPath);
            }
        }
        catch (Exception e)
        {
    ...
    private void ExportSheet2Csv(ISheet sheet, StreamWriter sw)
    {
        ...
        for (int i = 0; i < cellCount; i++)
        {
            ...
            string fieldName = GetCellString(sheet, 2, i);
            string fieldType = GetCellString(sheet, 0, i);
            cellInfos[i] = new CellInfo() { Name = fieldName, Type = fieldType, Desc = fieldDesc };
        }
        ...
        for (int i = 0; i <= sheet.LastRowNum; ++i)
        {
            ...
            for (int j = 0; j < cellCount; ++j)
            {
                ...
                string fieldValue = GetCellString(row, j);
                //string fieldValue = row?.GetCell(i)?.ToString();
    ...
    private void ExportAllCsvClass(string exportDir)
    {
        foreach (string filePath in Directory.GetFiles(ExcelPath))
        {
            if (Path.GetExtension(filePath) != ".xlsx")
            ...
            ExportCsvClass(filePath, exportDir);
        }
        Log.Debug("生成类完成!");
        AssetDatabase.Refresh();
    }

    private void ExportCsvClass(string fileName, string exportDir)
    {
        XSSFWorkbook xssfWorkbook;
        using (FileStream file = new FileStream(fileName, FileMode.Open, FileAccess.Read, FileShare.ReadWrite))
        {
        ...
        string protoName = Path.GetFileNameWithoutExtension(fileName);
        Log.Info($"{protoName}生成class开始");
        string exportPath = Path.Combine(exportDir, $"{protoName}Data.cs");
        using (FileStream txt = new FileStream(exportPath, FileMode.Create))
        using (StreamWriter sw = new StreamWriter(txt))
        {
            StringBuilder sb = new StringBuilder();
            ISheet sheet = xssfWorkbook.GetSheetAt(0);
            sb.Append("namespace ETHotfix\n{\n");
            ...
            sb.Append($"\tpublic class {protoName}Data\n");
            sb.Append("\t{\n");
            //sb.Append("\t\tpublic long id { get; set; }\n");
